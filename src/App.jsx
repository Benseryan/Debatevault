...
const GENERATE_PROMPT = `You are a world-class WSDC debate coach writing stock arguments for a debate resource database.
...
export default function App() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (data && data.length > 0) { setMotions(data); }
        else { setMotions([]); }
      } catch {
        setLoadError(true);
        setMotions([]);
  ...
  function checkApiKey() {
    ...
    else { alert("That doesn't look like an Anthropic API key. It should start with sk-ant"); }
  }
        ...
        <div style={{maxWidth:"780px",margin:"0 auto",padding:"40px 24px 60px"}}>
          ...
          {!adminUnlocked ? (
            ...
            <div style={{background:"#111125",border:"1px solid #2a2a4a",borderRadius:"14px",padding:"32px",maxWidth:"560px"}}>
              <p style={{fontSize:"15px",fontWeight:600,color:"#e0e0f0",marginBottom:"8px"}}>Paste your Supabase service key</p>
              <p style={{fontSize:"13px",color:"#a89aff",marginBottom:"16px"}}>Supabase → Settings → API → service_role key</p>
            ...
            <>
              {/* API Key setup (optional but shown prominently) */}
              {!apiKeySet && (
                <div style={{background:"#0d1a0d",border:"1px solid #2a4a2a",borderRadius:"12px",padding:"20px",marginBottom:"24px"}}>
                  <p style={{fontSize:"14px",fontWeight:600,color:"#40c090",marginBottom:"6px"}}>Connect AI Generation (optional)</p>
                  <p style={{fontSize:"13px",color:"#555",marginBottom:"12px"}}>Paste your Anthropic API key to enable one-click argument generation. Get one at console.anthropic.com</p>
                  <div style={{display:"flex",gap:"10px"}}>
                    <input type="password" style={{...INP,flex:1}} placeholder="sk-ant-..." value={apiKeyInput} onChange={e => setApiKeyInput(e.target.value)} />
                    <button onClick={checkApiKey} style={{padding:"10px 18px",borderRadius:"8px",border:"none",background:"#2a4a2a",color:"#40c090",fontSize:"13px",fontWeight:600,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}>Connect</button>
                  </div>
                </div>
              )}
              ...
              {adminTab === "add" && (
                <div style={{display:"flex",flexDirection:"column",gap:"18px"}}>
                  ...
                  <Lbl label="Keywords (comma separated)">
                  ...
                  <button onClick={generateArguments} disabled={generating || !apiKeySet}
                    style={{padding:"13px 28px",borderRadius:"10px",border:`1px solid ${apiKeySet?"#40c09066":"#333"}`,background:apiKeySet?"rgba(40,160,120,.15)":"#111",color:apiKeySet?"#40c090":"#555",fontSize:"14px",fontWeight:700,cursor:apiKeySet?"pointer":"not-allowed",fontFamily:"inherit",transition:"all .2s"}}>
